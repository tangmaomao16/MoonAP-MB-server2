...
\centering
\begin{tikzpicture}[
  layer/.style={draw, rounded corners, minimum width=12cm, minimum height=1cm, align=center},
  arrow/.style={-{Latex[length=2.5mm]}, thick}
]
...
\draw[arrow] (cloud.east) -- ++(0.7,0) |- node[pos=0.25,right]{install} (local.east);
\draw[arrow] (personal.west) -- ++(-0.7,0) |- node[pos=0.25,left]{save} (local.west);
\end{tikzpicture}
...
Figure~\ref{fig:skill-ui} shows the \skill{} interface in the browser. A user can select a Local-\skill{}-Hub directory, inspect installed \skill{}s, refresh the Cloud-\skill{}-Hub catalog, install public \skill{}s, and run local \skill{}s.

\begin{figure}[t]
\centering
\includegraphics[width=0.94\linewidth]{figures/screenshot-skill-hub.png}
\caption{\moonap{} \skill{} interface with Local-\skill{}-Hub and Cloud-\skill{}-Hub sections.}
\label{fig:skill-ui}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.94\linewidth]{figures/screenshot-runtime-skill.png}
\caption{Runtime result and reusable \skill{} flow. A generated application can be run, inspected, and saved for later reuse.}
\label{fig:runtime-skill}
\end{figure}
