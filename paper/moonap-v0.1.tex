...
\begin{abstract}
Large language models (LLMs) can synthesize useful programs from natural-language descriptions, but the path from generated code to a runnable, inspectable, and reusable application remains fragmented. This paper presents \moonap{} v0.1, a MoonBit-oriented agent playground that turns a natural-language application request into MoonBit source code, compiles it to \wasm{}, executes it locally in the browser, and optionally saves the successful artifact as a reusable \moonap{}-\skill{}. The system combines an LLM routing layer, a MoonBit compilation service, browser-local runtime contracts, and a three-layer \skill{} architecture for personal reuse, local installation, and cloud discovery. The v0.1 prototype demonstrates twelve task categories, including form-based calculators, JSON and text utilities, CSV file analysis, and browser-local large FastQ generation and analysis. We describe the system design, implementation, runtime contract abstraction, and lessons learned from building an end-to-end software synthesis workflow around a young programming language and browser-local execution. The central finding is not that current lightweight LLMs can already synthesize MoonBit reliably; rather, it is that a practical synthesis environment needs explicit runtime contracts, deterministic simulation paths, artifact reuse, and local execution boundaries to make generated applications usable.
\end{abstract}

\section{Introduction}

LLM-based software synthesis has made it possible for users to describe a program in natural language and receive executable source code~\cite{chen2021evaluating,austin2021program,li2022competition}. However, code generation alone is rarely a complete user workflow. A practical system must select or configure a model, preserve prompts and generated source, compile the program, expose runtime inputs, execute the artifact, present results, and allow successful programs to be reused. These requirements become more pronounced when the target language is new, when the generated program is intended to run in a browser, or when user data should remain local.
...
\begin{itemize}[leftmargin=*]
  ...
  \item browser-local large-file workflows for FastQ generation and analysis, where file contents stay in the browser;
...
Generated applications should run in the user's browser where possible. This is especially important for file-oriented workflows, where local files should not be sent to the LLM.

\paragraph{Reusable artifacts.}
A successful run should not be a one-off event. The generated application should be saved as a structured package that can be reopened and executed later.
...
\end{tikzpicture}
\caption{End-to-end \moonap{} workflow. The LLM participates in source generation, while browser-local execution and artifact reuse are controlled by the host system.}
\label{fig:pipeline}
\end{figure}
...
\moonap{}-\skill{}s are reusable application packages. A saved \skill{} contains a human-readable \texttt{SKILL.md}, generated MoonBit source, a compiled \wasm{} artifact, runtime metadata, and optional distribution files. The v0.1 design separates \skill{}s into three layers (Figure~\ref{fig:skill}).
...
This separation addresses two practical issues. First, it prevents cloud catalog entries from being treated as directly executable objects before they have been installed and inspected locally. Second, it gives the user a clear distinction between self-created artifacts and public artifacts obtained from a hub. In the browser UI, a user can select a Local-\skill{}-Hub directory, inspect installed \skill{}s, refresh the Cloud-\skill{}-Hub catalog, install public \skill{}s, and run local \skill{}s.
...
\moonap{} uses runtime contracts for this purpose. A runtime contract is a structured specification associated with a generated artifact. It can include:
...
The runtime contract enables generic execution paths for many small tools. For example, the Celsius/Fahrenheit converter, minutes/seconds converter, BMI calculator, circle calculator, loan payment calculator, tip calculator, JSON formatter, and text analyzer can all be represented as form or text contracts. The CSV analyzer uses a file contract with a browser-local file picker and report output.

The contract also makes \skill{} reuse possible. When a \skill{} is run again, \moonap{} does not need to ask the LLM to regenerate code. Instead, it reloads the saved runtime specification, renders the appropriate UI, and executes the local runtime path.

\section{Browser-Local Large-File Workflows}

\moonap{} v0.1 includes two large-file FastQ workflows: generation and analysis. These workflows are intentionally treated as high-value demonstrations of browser-local computation rather than as the only purpose of the platform.

\paragraph{Large FastQ generation.}
The generator uses a browser file-save picker and a streamed writer. The user chooses an output file, enters parameters such as target size, read length, random seed, and N-base rate, and the browser writes records locally. The LLM does not receive the generated file contents.

\paragraph{Large FastQ analysis.}
The analyzer uses a browser file-open picker and reads the selected file in chunks. It counts reads, lines, bases, base composition, malformed records, read lengths, and elapsed time. Again, file contents stay in the browser; the server receives only summary metrics and result metadata.

Figures~\ref{fig:fastq-report-summary} and~\ref{fig:fastq-report-bases} show the browser-local FastQ report generated by the analyzer. The report is opened from a browser blob URL and presents summary metrics and base-composition results without uploading the original file contents.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.94\linewidth]{figures/screenshot-fastq-report-summary.png}
\caption{FastQ analysis report summary. The browser-local analyzer reports file size, chunk count, request id, and high-level run metadata.}
\label{fig:fastq-report-summary}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=0.94\linewidth]{figures/screenshot-fastq-report-bases.png}
\caption{FastQ analysis base-composition section. The report includes per-base counts and percentages computed locally in the browser.}
\label{fig:fastq-report-bases}
\end{figure}
...
Users should not need to pay the latency and uncertainty cost of an LLM call every time they run a useful generated app. The \skill{} format turns synthesis into a reusable software artifact.
